...
df_comp = compinfo()
df_comp.columns = ["Symbol", "Market", "Name", "Sector", "Industry", "ListingDate", "SettleMonth", "Representative",
                   "HomePage", "Region"]
...
for scode in df_comp["Symbol"]:
    ...
    add_macd(df_stock, si.macd())
    add_rsi(df_stock, si.rsi())
    add_dmi(df_stock, si.dmi())
    add_boll(df_stock, si.boll())
    add_env(df_stock, si.env())
    add_obv(df_stock, si.obv())
